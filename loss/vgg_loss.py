...
class PerceptualVGGLoss(nn.Module):
    def __init__(self):
        super(PerceptualVGGLoss, self).__init__()
        self.vgg = vgg.vgg19(pretrained=True)
        self.vgg.eval()
        self.layers = [4, 9, 18, 27, 36] #[5,9,13,17] # # [5,9,13,17]
        self.layers = [36]
        self.hook_features = []
        self._feats = []
        ...
        for block_idx, block in enumerate(self.vgg.features.children()):
            if block_idx in self.layers:
                self.hook_features.append(block.register_forward_hook(self._get_hook()))

    def forward_one(self, gt):
        self._feats = []
        self.vgg.to(gt.device)
        _ = self.vgg(gt)
        return self._feats

    def forward(self, gt, recover):
        gt_fea = self.forward_one(gt)
        predict_fea = self.forward_one(recover)
        loss = 0
        for gt_f, predict_f in zip(gt_fea, predict_fea):
            loss += self.mse_loss(gt_f, predict_f)
        return loss
